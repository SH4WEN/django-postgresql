...
#         if form.is_valid():
#             new_product = form.cleaned_data['product']
#             if Product.objects.filter(product=new_product).exists():
#                 messages.error(request, "Product already exists")
#                 return redirect('add_product')  # Redirect back to the create product page
#             else:
#                 new_unit = form.cleaned_data['unit']
#                 new_description = form.cleaned_data['description']
#                 new_branch = form.cleaned_data['branch']
...
def add_supply_for_product(request, product_id):
    if not request.user.is_authenticated:
...
#         return redirect('login')  # Token is missing, redirect to login
#     token = request.session['token']
#     # You can also store the token in a database or cache to validate it
#     # For simplicity, we'll just check if the token exists in the session
#     if not token:
#         return redirect('login')  # Token is invalid, redirect to login
...
def index_supply (request):
    ...
    #     messages.error(request, "You must be Login")
    #     return redirect('login')  # Redirect to login page if user is not authenticated
    return render (request, 'supply/index_supply.html', {
